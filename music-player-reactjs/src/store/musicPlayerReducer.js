...
function getRandomNumber(arr, excludeNumber) {
  const randomNumber = Math.floor(Math.random() * arr.length);
  return arr[randomNumber] === excludeNumber
    ? getRandomNumber(arr, excludeNumber)
    : arr[randomNumber];
}
...
export default function musicPlayerReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case UPDATE_PLAY_LIST:
      return {
        ...state,
        playList: action.newPlayList,
        currentIndex: action.newPlayList.findIndex(
